...
class ModelPrevSegAndClickWrapper(nn.Module):
    def __init__(self, orig_network):
    ...
    def forward(self, *args, **kwargs):
        with torch.no_grad():
            ...
            n_clicks = np.random.choice(
                [0, 1, 2, 3, 4, 5], size=1, p=[0.75, 0.05, 0.05, 0.05, 0.05, 0.05]
            ).item()
            # print("n_clicks", n_clicks)

            if n_clicks > 0:
                seg_outputs = self.orig_network(x)
                final_seg_output = seg_outputs[0]
                final_seg_mask = (torch.diff(final_seg_output, dim=1) > 0).float()
                # print("final_seg_output", final_seg_output.shape)
                # print("x", x.shape)
                # print("final_seg_mask", final_seg_mask.shape)
